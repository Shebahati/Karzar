...
- [ ] **R9** OpenAPI contract drift undetected — Owner: Backend Architect — *mitigate:* `openapi/v1.json` is missing `/api/v1/products/slug/{slug}` (live since #126, snapshot last regenerated in #111); AODS `--gate openapi` now detects this; regenerate then make the gate blocking (AODS `CR-012`).
