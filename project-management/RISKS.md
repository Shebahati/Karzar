...
- [ ] **R5** Single VPS staging=live blast radius — Owner: Ops / Mohammad Shebahati — *mitigate:* `CR-011` Option B (2026-07-30) removed push auto-deploy; deploy only via `workflow_dispatch` + `RELEASE_PLAN.md` §4 rollback; residual until Option A (separate staging host).
- [ ] **R6** Scope creep vs mid-tail checkpoint KPI (knowledge platform and broad P2 work) — Owner: PMO — *mitigate:* REL-001 freeze list remains authoritative for checkpoint P0s; KB-001 schedule gate lifted 2026-07-30 but stays P2 — run only via dedicated SPEC/IMPL nodes, not as silent reopen of freeze.
...
- [x] **R9** OpenAPI contract drift undetected — Owner: Backend Architect — *closed 2026-07-30:* snapshot regenerated + Backend CI job `aods` runs `--gate openapi` + required on Protect main (`OI-GOV-02` CLOSED / AODS `CR-012` CLOSED).
- [ ] **R10** AI agents operating without enforced scope or context control — Owner: Owner — *mitigate:* AODS Accepted + kickoff gate + required CI `aods`; `CR-009` Option B (2026-07-30) forbids citing external `Website/docs/` as binding SoR; residual: agents inventing unpromoted Proposed packs (`CR-010`).
