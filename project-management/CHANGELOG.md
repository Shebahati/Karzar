...
## 2026-08-04
- [x] **IMG-02B** — Parallel live candidate discovery (IMG-02B-02..04) on `feat/img02b-parallel-source-discovery`. Dasqua 2/687, INSIZE 49/263 candidates (18 unique downloads), SAN OU 0/253. External roots under `/var/tmp/karzar-image-discovery/` and `/home/moahmmad/Projects/Karzar-image-discovery/IMG-02B`. rights_status=review_required; no DB/ProductImage mutations. Next: IMG-02B-05 human review.
...
- [x] **IMG-02B-01 R2** — Fail-closed hardening on PR #209: inventory presence contradictions rejected; `--allow-nonempty-output` removed; `--copy-final-to` never recursively deletes; review ZIP basename collisions rejected. Accepted 1204-product Artifact semantics unchanged. Live discovery still not started.
