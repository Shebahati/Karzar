...
## 2026-08-03
- [ ] **IMG-02A-01-R1** — Pre-authoritative boundary hardening for existing image audit (URL marker exactness, storage-index file meta, SQL guard, atomic publish, coverage/duplicate fixes). Branch `feat/existing-image-audit`. 43 tests; authoritative run BLOCKED.
- [ ] **IMG-02A-01** — Canonical Existing Product Image Inventory (read-only DB + storage scan). Branch `feat/existing-image-audit`. No mutations; remote URLs unverified; Draft PR.
