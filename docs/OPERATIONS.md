...
**Current (self-hosted local package — Prompt 85 cutover):** GitHub-hosted job enforces `main` + `KARZAR_DEPLOY_FREEZE` only. The `[self-hosted, karzar-vps]` package job fetches `/opt/karzar/mirror/Karzar.git`, checks out `github.sha` into workspace, copies tracked files into `/opt/karzar/incoming/<sha>/tree`, builds Storefront/Admin images on the VPS (`KARZAR_PACKAGE_DRY_RUN=0`), writes `deploy-manifest.sha256` + `HANDOFF_COMPLETE` (`transport=local-package`) and `FRONTEND_IMAGES_HANDOFF_COMPLETE` (`transport=local-package`). No GitHub→VPS SSH is used for source or image handoff.

The deploy job (also `karzar-vps`, `environment: staging`) verifies markers on disk, live-rsyncs backend/frontend, loads the prebuilt image bundle, rebuilds/restarts via compose scripts, and smokes. Job outputs are NOT used to carry manifest SHAs. Cleanup runs on `karzar-vps` after successful deploy only (local `cleanup-incoming-source.sh` with `SSH_HOST` unset). Failed deploys retain `/opt/karzar/incoming/<sha>` for diagnosis.

Host prep: `/opt/karzar/{incoming,workspace,mirror,logs/deploy}` owned by `github-runner` (`prepare-self-hosted-dirs.sh`). Scripts: [`package-incoming-local.sh`](../deploy/staging/scripts/package-incoming-local.sh), [`ensure-git-mirror.sh`](../deploy/staging/scripts/ensure-git-mirror.sh), [`verify-incoming-source.sh`](../deploy/staging/scripts/verify-incoming-source.sh), [`cleanup-incoming-source.sh`](../deploy/staging/scripts/cleanup-incoming-source.sh), [`build-staging-frontend-images.sh`](../deploy/staging/scripts/build-staging-frontend-images.sh), [`load-incoming-frontend-images.sh`](../deploy/staging/scripts/load-incoming-frontend-images.sh). Dry-run workflow (no live mutate): **Deploy Staging Self-Hosted Package Dry-Run**. Local selftests: `test-package-incoming-local.sh`, `test-delta-rsync-handoff.sh`, `cleanup-incoming-source.sh --selftest`, `test-prebuilt-frontend-handoff.sh`.

**Legacy (retired from Deploy Staging):** GitHub-hosted delta-rsync + FE image SSH push (`push-incoming-source.sh`, `push-incoming-frontend-images.sh`) remains in-tree for reference/selftests but is not invoked by `deploy-staging.yml` after Prompt 85. Historical notes: Actions suppressed cross-job `manifest_sha` (run `34039051081`); SSH TCP timeouts (Prompts 75–77).
