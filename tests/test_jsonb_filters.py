...
class TestJsonbFiltersPostgres:
    def test_postgres_exact_match_uses_contains(self):
        conditions = build_specification_filters(
            {"technical_specs.range": "0-150mm"},
            dialect_name="postgresql",
        )
        sql = str(conditions[0].compile(dialect=postgresql.dialect()))
        assert "@>" in sql
        assert "specifications" in sql.lower()
    ...
    def test_postgres_boolean_uses_contains(self):
        conditions = build_specification_filters(
