...
function resolveLocalBuyerShipping(): {
  address: string;
  postalCode: string;
} {
  const saved = useAddressStore.getState().getDefault();
  return formatBuyerAddressParts(saved);
}
...
function optionsFromSummary(
  ...
  const shipping = resolveLocalBuyerShipping();
  return {
    kind,
    buyerName: order.customer_full_name ?? null,
    buyerPhone: order.customer_phone ?? null,
    // Public track /orders/me omit shipping PII — use account address book when present.
    buyerAddress: shipping.address || null,
    buyerPostalCode: shipping.postalCode || null,
