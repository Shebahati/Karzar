...
const DEFAULT_SUBJECT = "پیام از صفحه اصلی";

/** High-contrast controls for the dark support surface. */
const darkFieldClass =
  "[&>span:first-child]:text-[#F0F0F0] [&>span[role=alert]]:text-[#ff9a9c]";

const darkInputClass =
  "h-12 w-full rounded-xl border border-white/18 bg-white/[0.07] px-4 text-base font-medium text-white outline-none transition placeholder:text-white/50 focus:border-[#D02327]/55 focus:bg-white/[0.1] focus:ring-2 focus:ring-[#D02327]/35";

const darkTextareaClass =
  "w-full rounded-xl border border-white/18 bg-white/[0.07] p-4 text-base font-medium text-white outline-none transition placeholder:text-white/50 focus:border-[#D02327]/55 focus:bg-white/[0.1] focus:ring-2 focus:ring-[#D02327]/35";
...
export function HomeContactSection() {
  ...
  const form = useForm<ContactValues>({
    ...
    defaultValues: { subject: DEFAULT_SUBJECT },
  });
  ...
  const onSubmit = form.handleSubmit((values) =>
    submit.mutate(values, {
      onSuccess: () => form.reset({ subject: DEFAULT_SUBJECT }),
    }),
  );
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute -start-24 top-0 h-64 w-64 rounded-full bg-[#D02327]/[0.12] blur-3xl"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -end-16 bottom-0 h-56 w-56 rounded-full bg-[#5E5F5E]/[0.14] blur-3xl"
      />
      <div
        aria-hidden
          ...
          {submit.isSuccess ? (
            <div className="flex min-h-[280px] flex-col items-center justify-center rounded-2xl bg-white/[0.06] px-5 py-10 text-center ring-1 ring-inset ring-white/12">
              <motion.span
                ...
                animate={motionSafe ? { scale: 1, opacity: 1 } : undefined}
            ...
            <form onSubmit={onSubmit} className="space-y-3.5" noValidate>
              ...
              <Field label="موضوع" error={errors.subject?.message} className={darkFieldClass}>
                <input className={darkInputClass} {...form.register("subject")} />
              </Field>
