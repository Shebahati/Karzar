...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    initialCategoryRef.current = product.category_id;
  }, [product, form]);

  useEffect(() => {
    if (!specTemplate || !numericCategoryId) return;
    if (initialCategoryRef.current === numericCategoryId) return;
  ...
  async function onSubmit(values: ProductFormValues) {
    ...
    try {
      const payload = toProductUpdatePayload(parsed.data, specTemplate ?? null);
      const updated = await updateProduct.mutateAsync({ id: productId, payload });
      toast.success("محصول به‌روزرسانی شد", {
        description: `«${updated.name}» ذخیره شد.`,
      });
      ...
      if (err instanceof ApiError) {
        ...
        toast.error("به‌روزرسانی محصول ناموفق بود", { description: err.message });
      } else {
  ...
  const pending = isSubmitting || updateProduct.isPending || productLoading;

  if (productLoading) {
    return <p className="p-8 text-center text-muted-foreground">در حال بارگذاری محصول...</p>;
  }

  if (!product) {
    return <p className="p-8 text-center text-destructive">محصول یافت نشد.</p>;
  }
