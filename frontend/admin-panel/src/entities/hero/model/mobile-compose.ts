import type { HeroBuilderConfig, MobileComposePreset } from "./types";

export type MobileDockScale = "sm" | "md" | "lg";

export interface MobileComposeView {
  config: HeroBuilderConfig;
  /** @deprecated Dock removed from Storefront mobile hero — preview API compat only */
...
function cloneConfig(config: HeroBuilderConfig): HeroBuilderConfig {
  return structuredClone(config);
}

/**
 * Maps desktop freeform layout into distinct mobile compositions (copy / media).
 * Dock is no longer part of the mobile hero — categories are a separate home section.
 */
export function composeHeroForMobile(
  source: HeroBuilderConfig,
  ...
  switch (preset) {
    ...
    case "dock-first": {
      // Legacy id — dock moved out of mobile hero → compact copy layout
