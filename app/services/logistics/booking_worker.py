...
async def _book_one(db: AsyncSession, shipment: Shipment) -> None:
    ...
    if original_status == ShipmentStatus.CREATION_UNCERTAIN.value:
        try:
        ...
        if lookup.found and lookup.booking:
            ...
            await db.flush()
            logger.info(
                "postex reconciled shipment_id=%s order_id=%s parcel=%s",
                shipment.id,
                order.id,
    ...
    try:
        ...
        shipment.booking_next_attempt_at = datetime.now(UTC) + timedelta(seconds=60)
        logger.warning(
            "postex create timeout shipment_id=%s order_id=%s — marked creation_uncertain",
            shipment.id,
            order.id,
        )
        await db.flush()
