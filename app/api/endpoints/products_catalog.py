...
@router.get(
    "/slug/{slug}",
    response_model=ProductDetailResponse,
    summary="Get product by public slug (EPIC 1 / RFC-004)",
)
async def read_product_by_slug(
    slug: str = Path(..., min_length=1, max_length=200, description="Product URL slug"),
    db: AsyncSession = Depends(get_db),
    current_user: User | None = Depends(get_optional_current_user),
):
    """Storefront canonical resolve — must be registered before `/{product_id}`."""
    from app.crud import product as crud_product
